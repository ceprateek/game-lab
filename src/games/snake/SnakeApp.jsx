...
function DifficultySelect() {
  const selectDifficulty = useSnakeStore((s) => s.selectDifficulty)
  const bestScores = useSnakeStore((s) => s.bestScores)
  ...
  const difficultyOptions = [
    {
      key: 'easy',
      label: 'Easy',
      desc: 'Slower speed, no obstacles',
    ...
    },
    {
      key: 'medium',
      label: 'Medium',
      desc: 'Faster speed, some obstacles',
    ...
    <div className="h-full w-full relative overflow-hidden">
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
        <defs>
          ...
          <radialGradient id="snake-glow" cx="0.5" cy="0.35" r="0.4">
            <stop offset="0%" stopColor="#22c55e" stopOpacity="0.15" />
            <stop offset="100%" stopColor="#22c55e" stopOpacity="0" />
          </radialGradient>
        </defs>
        <rect width="400" height="800" fill="url(#snake-bg)" />
        <rect width="400" height="800" fill="url(#snake-glow)" />
      </svg>
      ...
      <div className="relative z-10 h-full flex flex-col px-6 pt-5">
        ...
        >
          ...
          <h1 className="text-3xl font-extrabold text-white mb-1">Snake</h1>
          <p className="text-green-300/50 text-sm">Slither & Grow</p>
        </motion.div>
...
export default function SnakeApp() {
  const screen = useSnakeStore((s) => s.screen)
  ...
  return (
    <div className="h-full w-full overflow-hidden relative">
      {screen === 'playing' && (
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
          <defs>
            <linearGradient id="snake-play-bg" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#0f172a" />
              <stop offset="100%" stopColor="#14532d" />
            </linearGradient>
          </defs>
          <rect width="400" height="800" fill="url(#snake-play-bg)" />
        </svg>
      )}
