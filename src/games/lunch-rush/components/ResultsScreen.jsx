  ...
  return (
    <div className="h-full w-full relative overflow-hidden">
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
        <defs>
          <radialGradient id="lr-result-glow" cx="0.5" cy="0.3" r="0.6">
        ...
        <rect width="400" height="800" fill="url(#lr-result-glow)" />
      </svg>
          ...
          >
            <div
              className={`w-2.5 h-2.5 rounded-sm ${
                ['bg-orange-400', 'bg-amber-400', 'bg-yellow-400', 'bg-red-400', 'bg-pink-400'][i % 5]
              }`}
              style={{ opacity: 0.6 }}
            />
          </motion.div>
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center px-6">
        ...
        >
          {won ? (stars === 3 ? '\u{1F3C6}' : stars === 2 ? '\u{1F389}' : '\u{1F44F}') : '\u{1F614}'}
        </motion.div>
