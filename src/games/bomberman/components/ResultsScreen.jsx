...
export default function ResultsScreen() {
  const { result, difficulty, resetGame, goToDifficulty } = useBombermanStore()
  const { score, enemiesKilled, wallsDestroyed, time, stars, won } = result || {}
  ...
  return (
    <div className="h-full w-full relative overflow-hidden">
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
        <defs>
          <radialGradient id="bomb-result-glow" cx="0.5" cy="0.3" r="0.6">
            <stop offset="0%" stopColor={won ? '#f97316' : '#ef4444'} stopOpacity="0.2" />
            <stop offset="50%" stopColor={won ? '#ea580c' : '#dc2626'} stopOpacity="0.05" />
            <stop offset="100%" stopColor="#0f172a" stopOpacity="0" />
          </radialGradient>
        </defs>
        <rect width="400" height="800" fill="#0f172a" />
        <rect width="400" height="800" fill="url(#bomb-result-glow)" />
      </svg>
          ...
          >
            <div
              className={`w-2.5 h-2.5 rounded-sm ${
                ['bg-orange-400', 'bg-amber-400', 'bg-red-400', 'bg-yellow-400', 'bg-rose-400'][i % 5]
              }`}
              style={{ opacity: 0.6 }}
            />
          </motion.div>
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center px-6">
        ...
        <motion.p
          ...
          className="text-orange-200/50 text-sm mb-5 capitalize"
        >
        ...
        >
          ...
          <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl px-4 py-3 text-center">
            <p className="text-white text-xl font-bold">{enemiesKilled}</p>
            <p className="text-white/40 text-xs">Enemies</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl px-4 py-3 text-center">
            <p className="text-white text-xl font-bold">{wallsDestroyed}</p>
            <p className="text-white/40 text-xs">Walls</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl px-4 py-3 text-center">
