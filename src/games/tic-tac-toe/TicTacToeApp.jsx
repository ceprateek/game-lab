...
function DifficultySelect() {
  const selectDifficulty = useTicTacToeStore((s) => s.selectDifficulty)
  const bestScores = useTicTacToeStore((s) => s.bestScores)
  ...
  return (
    <div className="h-full w-full relative overflow-hidden">
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
        <defs>
          <linearGradient id="ttt-bg" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#0f172a" />
            <stop offset="50%" stopColor="#1e1b4b" />
            <stop offset="100%" stopColor="#0f172a" />
          </linearGradient>
          <radialGradient id="ttt-glow" cx="0.5" cy="0.35" r="0.4">
            <stop offset="0%" stopColor="#6366f1" stopOpacity="0.15" />
            <stop offset="100%" stopColor="#6366f1" stopOpacity="0" />
          </radialGradient>
        </defs>
        <rect width="400" height="800" fill="url(#ttt-bg)" />
        <rect width="400" height="800" fill="url(#ttt-glow)" />
      </svg>
      ...
      <div className="relative z-10 h-full flex flex-col px-6 pt-5">
        ...
        >
          ...
          <p className="text-indigo-300/50 text-sm">Challenge the AI</p>
        </motion.div>
...
export default function TicTacToeApp() {
  const screen = useTicTacToeStore((s) => s.screen)
  ...
  return (
    <div className="h-full w-full overflow-hidden relative">
      ...
      {screen === 'playing' && (
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
          <defs>
            <linearGradient id="ttt-play-bg" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#0f172a" />
              <stop offset="100%" stopColor="#1e1b4b" />
            </linearGradient>
          </defs>
          <rect width="400" height="800" fill="url(#ttt-play-bg)" />
        </svg>
      )}
