...
export default function ResultsScreen() {
  const { winner, score, stars, difficulty, resetGame, goToDifficulty } = useTicTacToeStore()

  const emoji = winner === 'X' ? '🏆' : winner === 'O' ? '🤖' : '🤝'
  const title = winner === 'X' ? 'You Win!' : winner === 'O' ? 'AI Wins!' : "It's a Draw!"
  const subtitle = winner === 'X'
    ? 'Great strategy!'
    : winner === 'O'
  ...
  return (
    <div className="h-full w-full relative overflow-hidden">
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice">
        <defs>
          <radialGradient id="ttt-victory-glow" cx="0.5" cy="0.3" r="0.6">
            <stop offset="0%" stopColor="#6366f1" stopOpacity="0.2" />
            <stop offset="50%" stopColor="#4338ca" stopOpacity="0.05" />
            <stop offset="100%" stopColor="#0f172a" stopOpacity="0" />
          </radialGradient>
        </defs>
        <rect width="400" height="800" fill="#0f172a" />
        <rect width="400" height="800" fill="url(#ttt-victory-glow)" />
      </svg>
        ...
        >
          <div
            className={`w-2.5 h-2.5 rounded-sm ${
              ['bg-indigo-400', 'bg-rose-400', 'bg-cyan-400', 'bg-amber-400', 'bg-emerald-400'][i % 5]
            }`}
            style={{ opacity: 0.6 }}
          />
        </motion.div>
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center px-6">
        ...
        <motion.p
          ...
          className="text-indigo-200/50 text-sm mb-5 capitalize"
        >
          {subtitle}
        </motion.p>
        ...
        >
          ...
          <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl px-5 py-3 text-center">
            <p className="text-white text-xl font-bold capitalize">{difficulty}</p>
